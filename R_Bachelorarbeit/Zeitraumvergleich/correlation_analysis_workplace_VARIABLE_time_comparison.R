...
for ( bundesland in bundeslaender) 
{
  ...
  for (i in 10:15) {
    ...
    #add rows
    dataframe_final [i_counter,] <- c(year, from_date1, to_date1, bundesland, "workplaces_percent_change_from_baseline", colnames(dataset1)[i], a$estimate, a$p.value, population_density[, bundesland])
  ...
  for (i in 10:15) {
    ...
    dataframe_final [i_counter,] <- c(year, from_date2, to_date2, bundesland, "workplaces_percent_change_from_baseline", colnames(dataset2)[i], b$estimate, b$p.value, population_density[, bundesland])
    i_counter=i_counter+1
  }
...
write.csv(dataframe_final,"/Users/Aleks/Desktop/Studium/Bachelorarbeit/DataAnalytics/R projects/R_Bachelorarbeit/Zeitraumvergleich/dataframe_final_export.csv", row.names = FALSE)
