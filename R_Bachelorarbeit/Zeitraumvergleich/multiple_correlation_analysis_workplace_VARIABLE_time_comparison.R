...
dataframe_final <- data.frame(matrix(ncol = 12, nrow = 0))
colnames(dataframe_final) <-c("year", "from", "to", "bundesland", "variable_x", "variable_y", "R-value", "p_value_x1", "p_value_x2", "vif","f-statistic", "population_density" )

i_counter=1
f_statistic_p_value=0
...
for ( bundesland in bundeslaender) 
{
  ...
  for (i in 10:15) {
    ...
    multipleLinearModel1 <- lm(dataset1[,i] ~ dataset1$workplaces_percent_change_from_baseline + dataset1$residential_percent_change_from_baseline, data = dataset1)
    
    f_statistic_p_value=format.pval(pf(summary(multipleLinearModel1)$fstatistic[1L],
                   summary(multipleLinearModel1)$fstatistic[2L],
                   summary(multipleLinearModel1)$fstatistic[3L],
                   lower.tail=FALSE))
    ...
    vif_variable<-vif(multipleLinearModel1)
    
    print(vif(multipleLinearModel1))
   
    

    print(paste(colnames(dataset1)[i], " R:", summary(multipleLinearModel1)$r.squared, " p-value:", summary(multipleLinearModel1)$coefficients[ , 4] ))
  
    
    dataframe_final [i_counter,] <- c(year, from_date1, to_date1, bundesland, "workplaces_percent_change_from_baseline + residential_percent_change_from_baseline", colnames(dataset1)[i], summary(multipleLinearModel1)$r.squared, summary(multipleLinearModel1)$coefficients[ 2, 4], summary(multipleLinearModel1)$coefficients[ 3, 4],vif(multipleLinearModel1)[1],f_statistic_p_value, population_density[, bundesland])
  ...
  for (i in 10:15) {
    ...
    multipleLinearModel2 <- lm(dataset2[,i] ~ dataset2$workplaces_percent_change_from_baseline + dataset2$residential_percent_change_from_baseline, data = dataset2)
    
    f_statistic_p_value=format.pval(pf(summary(multipleLinearModel2)$fstatistic[1L],
                                       summary(multipleLinearModel2)$fstatistic[2L],
                                       summary(multipleLinearModel2)$fstatistic[3L],
                                       lower.tail=FALSE))
   ...
   dataframe_final [i_counter,] <- c(year, from_date2, to_date2, bundesland, "workplaces_percent_change_from_baseline + residential_percent_change_from_baseline", colnames(dataset2)[i], summary(multipleLinearModel2)$r.squared, summary(multipleLinearModel2)$coefficients[ 2, 4], summary(multipleLinearModel2)$coefficients[ 3, 4],vif(multipleLinearModel2)[1],f_statistic_p_value, population_density[, bundesland])
   
   print(paste(colnames(dataset2)[i], " R:", summary(multipleLinearModel2)$r.squared, " p-value:", summary(multipleLinearModel2)$p.value)) 

   print(vif(multipleLinearModel2))
...
write.csv(dataframe_final,"/Users/Aleks/Desktop/Studium/Bachelorarbeit/DataAnalytics/R projects/R_Bachelorarbeit/Zeitraumvergleich/multiple_dataframe_final_export.csv", row.names = FALSE)
