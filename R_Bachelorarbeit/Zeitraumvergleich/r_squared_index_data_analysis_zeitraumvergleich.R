...
dataframe=read.csv("/Users/Aleks/Desktop/Studium/Bachelorarbeit/DataAnalytics/R projects/R_Bachelorarbeit/Zeitraumvergleich/r_squared_corr_index_var_dataframe_final_export.csv", header = TRUE, sep = ",")
...
for (bundesland_iterator in bundeslaender)
{
  ...
  dataset_bundesland=filter(dataframe, bundesland==bundesland_iterator)
...
plot_list[[i]]<-ggplot(dataset_bundesland, aes(fill=paste(from,"-", to), y=R.value, x=variable_y)) + ggtitle( paste("R-Values for dependend variables (", bundesland_iterator, ")"))+
  geom_bar(position="dodge", stat="identity")+labs(fill="Timespan")+ theme(panel.background=element_blank(), panel.grid.minor = element_line(colour="black"), legend.text=element_text(size=5),
    axis.title.x = element_blank(), axis.text.x = element_text(face="bold", size=3.5))+
...
pdf("/Users/Aleks/Desktop/Studium/Bachelorarbeit/DataAnalytics/R projects/R_Bachelorarbeit/Zeitraumvergleich/r_squared_index_plots.pdf", width=811/72, height=478/72)
