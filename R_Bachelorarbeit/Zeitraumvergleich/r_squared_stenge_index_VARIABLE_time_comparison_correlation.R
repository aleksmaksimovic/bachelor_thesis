...
for ( bundesland in bundeslaender) 
{
  ...
  for (i in 10:15) {
    ...
    linearModel1 <- lm(dataset_merge[,i] ~ dataset_merge$bl_mn_idx_t, data = dataset_merge)
    
    a <- cor.test(dataset_merge$bl_mn_idx_t, dataset_merge[,i])
    print(paste(colnames(dataset_merge)[i], " R:", summary(linearModel1)$r.squared, " p-value:", a$p.value))
    #add rows
    dataframe_final [i_counter,] <- c(year, from_date1, to_date1, bundesland, "measure_index", colnames(dataset_merge)[i], summary(linearModel1)$r.squared, a$p.value, population_density[, bundesland])
  ...
  for (i in 10:15) {
    ...
    linearModel2 <- lm(dataset_merge[,i] ~ dataset_merge$bl_mn_idx_t, data = dataset_merge)
    
    b <- cor.test(dataset_merge$bl_mn_idx_t, dataset_merge[,i])
    print(paste(colnames(dataset_merge)[i], " R:", summary(linearModel1)$r.squared, " p-value:", b$p.value))
    dataframe_final [i_counter,] <- c(year, from_date2, to_date2, bundesland, "measure index", colnames(dataset_merge)[i], summary(linearModel1)$r.squared, b$p.value, population_density[, bundesland])
    i_counter=i_counter+1
  }
...
write.csv(dataframe_final,"/Users/Aleks/Desktop/Studium/Bachelorarbeit/DataAnalytics/R projects/R_Bachelorarbeit/Zeitraumvergleich/r_squared_corr_index_var_dataframe_final_export.csv", row.names = FALSE)
