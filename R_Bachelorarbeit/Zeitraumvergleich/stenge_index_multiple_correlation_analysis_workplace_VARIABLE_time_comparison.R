...
for ( bundesland in bundeslaender) 
{
  ...
  for (i in 10:15) {
    ...
    multipleLinearModel1 <- lm(dataset_merge[,i] ~ dataset_merge$workplaces_percent_change_from_baseline + dataset_merge$bl_mn_idx_t, data = dataset_merge)
    ...
    print(paste(colnames(dataset_merge)[i], " R:", summary(multipleLinearModel1)$r.squared, " p-value:", summary(multipleLinearModel1)$coefficients[ , 4] ))
  
    
    dataframe_final [i_counter,] <- c(year, from_date1, to_date1, bundesland, "workplaces_percent_change_from_baseline + bl_mn_idx_t", colnames(dataset_merge)[i], summary(multipleLinearModel1)$r.squared, summary(multipleLinearModel1)$coefficients[ 2, 4], summary(multipleLinearModel1)$coefficients[ 3, 4],vif(multipleLinearModel1)[1],f_statistic_p_value, population_density[, bundesland])
  ...
  for (i in 10:15) {
    ...
    multipleLinearModel2 <- lm(dataset_merge[,i] ~ dataset_merge$workplaces_percent_change_from_baseline + dataset_merge$bl_mn_idx_t, data = dataset_merge)
   ...
   dataframe_final [i_counter,] <- c(year, from_date2, to_date2, bundesland, "workplaces_percent_change_from_baseline + bl_mn_idx_t", colnames(dataset_merge)[i], summary(multipleLinearModel2)$r.squared, summary(multipleLinearModel2)$coefficients[ 2, 4], summary(multipleLinearModel2)$coefficients[ 3, 4],vif(multipleLinearModel2)[1],f_statistic_p_value, population_density[, bundesland])
   
   print(paste(colnames(dataset_merge)[i], " R:", summary(multipleLinearModel2)$r.squared, " p-value:", summary(multipleLinearModel2)$p.value)) 
...
write.csv(dataframe_final,"/Users/Aleks/Desktop/Studium/Bachelorarbeit/DataAnalytics/R projects/R_Bachelorarbeit/Zeitraumvergleich/index_multiple_dataframe_final_export.csv", row.names = FALSE)
