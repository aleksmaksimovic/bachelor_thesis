...
data_frame_workplaces_change_berlin <- dataset21[dataset21$sub_region_1=="Berlin",]

#Merge dataframes dataset21 and dataset_infections_p_year
dataset_merged=merge(x=data_frame_workplaces_change_berlin, y=dataset_infections_p_year,by="date")

#Simple linear correlation

linModel <- lm(residential_percent_change_from_baseline~incidence_number, data = dataset_merged)

attributes(linModel) 

plot(residential_percent_change_from_baseline~incidence_number, data = dataset_merged,
...
y=dataset_merged$residential_percent_change_from_baseline
