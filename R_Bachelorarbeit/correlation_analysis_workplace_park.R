...
data_frame_berlin<-data_frame_berlin%>%
  select(date, workplaces_percent_change_from_baseline, parks_percent_change_from_baseline)
...
data_frame_berlin<-data_frame_berlin%>%
  select(workplaces_percent_change_from_baseline, parks_percent_change_from_baseline)

linModel <- lm(parks_percent_change_from_baseline~workplaces_percent_change_from_baseline, data = data_frame_berlin)

attributes(linModel) 

plot(parks_percent_change_from_baseline~workplaces_percent_change_from_baseline, data = data_frame_berlin,
     xlab = "workplace change from baseline",
     ylab = "parks change from baseline",
     main = "Correlation between workplace and parks change relative to the baseline"
)
...
y=data_frame_berlin$parks_percent_change_from_baseline

cor(x,y)

text(-60, 200, paste("r:",round(cor(x,y), digits=2)))
